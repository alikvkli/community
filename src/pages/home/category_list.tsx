    ...
    return (
        <section className="flex flex-col items-center gap-4 p-4">
            <div className="flex items-center justify-between w-full">
                ...
                <Link to="/search" className="text-vf-red underline flex items-center gap-2">
                    All
            ...
            <div className="flex items-start gap-2 overflow-x-auto w-full">
                {categoryListData.map(({ id, image, name, slug }, key) => (
                    <Link to={`/category/${slug}`} key={`${key}${id}`} className="flex flex-col items-center gap-3">
                        <img src={image} className="min-w-[70px] h-[70px] rounded-[16px] p-[2px] object-cover border-[2px] bg-gradient-to-r from-[#820000] to-[#E60000] " />
