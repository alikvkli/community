    ...
    return (
        <section className="flex flex-col items-center gap-4 p-4">
            <div className="flex items-center justify-between w-full">
                ...
                <button onClick={() => dispatch(setSystemNotification(true))} className="text-vf-red underline flex items-center gap-2">
                    All
            ...
            <div className="flex items-start gap-2 overflow-x-auto w-full h-full">
                {interestedServiceData.map((service) => (
                    <div key={service.id} className="flex flex-col h-fit items-center shadow-md p-4 rounded-xl bg-white mb-4 border border-[#ebebeb]">
                        <img src={service.image} />
                        <p>{service.name}</p>
                        <button onClick={() => dispatch(setSystemNotification(true))} className="bg-vf-red rounded-full flex items-center justify-center w-8 h-8">
